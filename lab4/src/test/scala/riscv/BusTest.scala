...
import bus.{AXI4LiteMasterBundle, AXI4LiteSlaveBundle}
import bus._
import chisel3._
import chiseltest._
...
class MemoryTestF extends AnyFlatSpec with ChiselScalatestTester {
  ...
  it should "perform read and write" in {
    test(new MemoryTest).withAnnotations(TestAnnotations.annos) { c =>
      ...
      FlexibleTestHelper.init_write_transaction(c.io.bundle, c.clock, 0x8.U, 0xDEADBEEFL.U, 0xF.U)
      ...
      FlexibleTestHelper.init_read_transaction(c.io.bundle, c.clock, 0x8.U)
...
class ROMLoaderTestF extends AnyFlatSpec with ChiselScalatestTester {
 ...
 └──────────────────┘     ├───────────┘              └───────┤     ├──┬─────┘  │ 
  ...
  class ROMLoaderTest extends Module with RequireAsyncReset {
    val io = IO(new Bundle {
...
class FunctionalTest extends AnyFlatSpec with ChiselScalatestTester {
  ...
  class TestBox extends Module with RequireAsyncReset {
    val io = IO(new Bundle {
  ...
  // this test also benchmark cycles of write transaction
